<div class="min-h-screen bg-white pb-32">
                        ...
                        <div class="flex items-center justify-end">
                            <div class="flex items-center gap-2">
                                ...
                                <button wire:click="decrementQuantity({{ $item['id'] }})"
                                    class="w-8 h-8 flex items-center justify-center border-2 border-gray-900 rounded-lg hover:bg-gray-50">
    ...
    @if (count($cartItems) > 0)
        <div class="fixed bottom-0 left-0 right-0 bg-white py-2 px-3 shadow-xl 
            ...
            <div class="max-w-2xl mx-auto">
                ...
                @else
                    <!-- Promo Code Input -->
                    <div class="flex gap-2 mb-4 bg-white py-2 px-3 shadow-xl 
             rounded-3xl border border-black">
                        ...
                        <button wire:click="applyPromoCode" wire:loading.attr="disabled"
                            class="px-6 py-2 bg-primary text-white rounded-lg text-sm font-semibold hover:bg-green-700 disabled:opacity-50">
                    ...
                    <button wire:click="goToCoupons"
                        class="w-full mb-4 py-2 border-2 border-primary text-primary rounded-lg text-sm font-semibold hover:bg-green-50 transition-colors flex items-center justify-center gap-2">
                ...
                <div class="space-y-2 mb-4">
                    ...
                    @if ($discount > 0)
                        <div class="flex justify-between text-sm">
                            ...
                            <span
                                class="font-semibold text-green-600">-Rp{{ number_format($discount, 0, ',', '.') }}</span>
                        </div>
                    @endif
                ...
                <button wire:click="proceedToCheckout" wire:loading.attr="disabled"
                    class="w-full bg-primary text-white py-3 rounded-lg font-semibold hover:bg-green-800 transition-colors disabled:opacity-50">
    ...
    @if ($showRemoveModal && $itemToRemove)
        <div class="fixed inset-0 z-50 overflow-y-auto">
            ...
            <div class="flex min-h-screen items-end justify-center p-0 md:items-center md:p-4">
                <div class="relative w-full max-w-md bg-white rounded-t-3xl md:rounded-2xl shadow-2xl transform transition-all"
                    @click.stop>
                    ...
                    <div class="px-6 py-6">
                        <div class="flex gap-4 items-center mb-4">
                            ...
                            <div class="flex-1">
                                <h4 class="text-sm font-medium text-gray-900 mb-1">{{ $itemToRemove->product->name }}
                                </h4>
                                <p class="text-xs text-gray-500 mb-1">
                                    Size: {{ $itemToRemove->product->unit ?? '-' }} | Qty:
                                    {{ $itemToRemove->quantity }}pcs
                                </p>
                                <p class="text-sm font-bold text-gray-900">Rp
                                    {{ number_format($itemToRemove->price, 0, ',', '.') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-center gap-4 mb-6">
                            ...
                            <span
                                class="text-lg font-bold text-gray-900 min-w-[2rem] text-center">{{ $itemToRemove->quantity }}</span>
